...
Fyrir hvert element skilgreinir staðallinn nokkuð nákvæmlega hvernig elementið hagar sér og hvernig eigi að útfær. Við sem _höfundar_[^1] vefja þurfum að skoða skilgreiningu á því hvernig á að nota, þá helst:
...
* [`<title>`](https://html.spec.whatwg.org/multipage/semantics.html#the-title-element) er það element sem ætti alltaf að skilgreina til að setja titil skjals.
...
Þegar við höfum sjálfstætt svæði innan skjals sem við munum vilja vísa í úr öðrum stöðum (hugsanlega með útskýringartexta), getum við notað [`<figure>`](https://html.spec.whatwg.org/multipage/grouping-content.html#the-figure-element). Þetta gæti verið skýringarmynd, graf, eða forritsskóði.
...
* [`<article>`](https://html.spec.whatwg.org/multipage/sections.html#the-article-element), skilgreinir sjálfstætt efnisatriði innan síðu. Þetta getur átt við grein, athugasemd, eða sjálfstæð virkni innan síðu. Hvert og eitt `<article>` element getur innihaldið `<header>` og `<footer>` element til að skilgreina fyrirsagnir og aukagögn.
* [`<aside>`](https://html.spec.whatwg.org/multipage/sections.html#the-aside-element), skilgreinir efni sem er að einhverju leiti skylt meginmáli en á þó ekki við í því. Þetta gæti verið annað efni á vefnum sem er óbeint tengt efni síðu, auglýsing sem fylgir efninu, eða leiðarvísi um efni greinar eða útdregin tilvitnun (e. pull quote).
...
Helstu orðalags element eru:

* [`<a>`](https://html.spec.whatwg.org/multipage/text-level-semantics.html#the-a-element), skilgreinir tengil ef `href` attribute er skilgreint, annars stakkt „akkeri“ í texta.
...
* [`<data>`](https://html.spec.whatwg.org/multipage/text-level-semantics.html#the-data-element), skilgreinir gagnastak sem bæði er lesanlegt af manneskjum (sem texti innan elements) og tölvum (sem gildi í `value` atrribute).
...
* [`<small>`](https://html.spec.whatwg.org/multipage/text-level-semantics.html#the-small-element), skilgreinir texta sem er „smáaletur“ eða álíka.
...
* [`<u>`](https://html.spec.whatwg.org/multipage/text-level-semantics.html#the-u-element), skilgreinir texta sem fær áherslu án þess að breyta textanum, t.d. til að benda á ritvillu. Stóð áður yfir _underline_, ætti að forðast þar sem sjálfgefna birting er undirstrikun sem gæti valdið rugling á textanum og tengli.
