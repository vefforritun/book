...
[HTML staðalinn](https://html.spec.whatwg.org/) skilgreinir lögleg element og gefur þeim merkingu (e. semantics). Vafrar og önnur forrit sem birta HTML nota þennan staðal og skilgreiningar til þess að birta HTML skjöl. Elementum er gróflega skipt í hópa eftir notkun, þar sem hvert element getur verið í einum eða fleiri hópum:
...
* [Embedded](https://html.spec.whatwg.org/multipage/dom.html#embedded-content-2) – innfellt
* [Interactive](https://html.spec.whatwg.org/multipage/dom.html#interactive-content) – gagnvirkt
...
Til að mynda tengingu við önnur gögn notum við [`<link>`](https://html.spec.whatwg.org/multipage/semantics.html#the-link-element). Ef `rel` attribute er sett þá einskorðast `<link>` við `<head>`, oftast með `href` sem vísar í skjal, t.d.
...
* `rel="preload"` skilgreinir að merkt gagn _verði_ að vera sótt sem fyrst, gerð gagnsins er skilgreint með `as` attribute. Með því getum við t.d. skipað vafra að sækja letur[^2] sem notað er sem fyrst og þannig látið það birtast fyrr en seinna (annars væri letrið sótt eftir CSS þáttun.)
...
Flest element sem við notum í meginmáli vefs, `<body>`, eru _flæði_ element, mörg þeirra eru einnig _greinilegt_ efni (e. palpable content): þau innihalda eitthvað sem birtist. Nokkur element falla aðeins undir þessa flokka og enga aðra:
...
[^2]: við sjáum hvernig letrið er notað í yfirferð á CSS.
